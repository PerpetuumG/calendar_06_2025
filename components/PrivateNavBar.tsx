...
const PrivateNavBar = () => {
  ...
  return (
    <nav
      className={
        'flex justify-between items-center fixed z-50 w-full h-28 bg-gray-200 px-10 gap-4 shadow-2xl mb-28'
      }
    >
      <Link href={'/events'} className={'flex items-center gap-1 hover:scale-150 duration-500'}>
        <Image src={'/assets/logo.svg'} alt={"Let's talk"} width={60} height={60} />
      </Link>
          ...
          {PrivateNavLinks.map((item) => {
            ...
            return (
              ...
              >
                ...
                <p className={cn('text-lg font-semibold max-lg:hidden')}>{item.label}</p>
              </Link>
            );
          })}
        </div>
